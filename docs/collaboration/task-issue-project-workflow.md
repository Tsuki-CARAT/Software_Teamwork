...
- Issue 标题和正文都使用中文，标题格式为 `[A/B/C/F/S-001] 中文任务标题`。
...
- `任务信息` 字段完整，尤其是 `状态`、`主责小组`、`优先级`、`批次`、`模块`、`预期工时`、`实际工时`、`Risk`、`GitHub Project`、`Project sync`。
...
- 正文包含可解析的 `状态`、`优先级`、`批次`、`模块`、`预期工时`、`实际工时`、`Risk`、`依赖任务` 等字段。
...
| 同步 `ExpectedHours`、`ActualHours` | Issue 正文 `预期工时` 和 `实际工时` 字段。 |
...
- 检查 GitHub Project 是否存在 `ExpectedHours` 和 `ActualHours` 文本字段。
...
自动化会把正文 `实际工时` 改为评论值，并同步 Project `ActualHours`。只有仓库维护者、协作者或当前 Assignee 可以通过评论设置实际工时。
...
- [ ] `预期工时` 已填写；暂不能估算时写 `待估`。
- [ ] `实际工时` 初始可写 `未填写`，完成后通过评论 `实际工时：2h` 回填。
