...
| 手工 smoke | `FILE_STORAGE_BACKEND=local go run ./cmd/server` 后上传/读取/删除 | pass（本次执行：healthz、上传、读取、删除、删除后 404） | local 路径仍可用于无 MinIO 的快速验证；真实 MinIO 路径使用 env-gated 联合 smoke。 |
